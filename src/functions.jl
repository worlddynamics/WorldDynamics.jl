...
Return `returnifgte` if the value `inputvalue` is greater than the threshold `threshold`, `returniflt` otherwise. This function correspond to the `CLIP` (also called `FIFGE`) function in the `DYNAMO` language.
"""
...
Return `0` if the value `inputvalue` is smaller than the threshold `threshold`, `returnifgte` otherwise. This function correspond to the `STEP` function in the `DYNAMO` language.
"""
...
Return `returnifzero` if the value `inputvalue` is approximately `0` with tolerance `1e-16`, `returnifnotzero` otherwise. This function correspond to the `SWITCH` (also called `FIFZE`) function in the `DYNAMO` language.
"""
